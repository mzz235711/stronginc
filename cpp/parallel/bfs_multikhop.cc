...
void Bfs_Multikhop::bfs_multikhopparallel(Fragment &fragment, Graph &dgraph,std::unordered_set<VertexID> &nodeset, std::unordered_set<VertexID> &root, int bound){
...
for (auto v : root){
    if (fragment.isInnerVertex(v)){
        que.emplace(fragment.getLocalID(v), bound);
    }
}
...
void Bfs_Multikhop::incEval(Fragment &fragment, Graph &dgraph,std::unordered_set<VertexID> &nodeset, std::unordered_map<VertexID,int> &DistInf, std::unordered_set<VertexID> &root, int bound){
std::queue<std::pair<VertexID,int>> que;
...
bool Bfs_Multikhop::is_continue(){
...
std::unordered_set<VertexID> Bfs_Multikhop::assemble_result(std::unordered_set<VertexID> &nodeset){
