...
class Dual_Inc_Exp {
 public:
  ...
  void get_qeury_efile(int index, std::string &query_efile) {
  ...
  void Run() {
    ...
    dgraph_loader.LoadGraph(dgraph, this->graph_vfile, this->graph_efile);
    stop_timer(LOAD_TIMER);
    for (int index = 0; index < query_num; index++) {
      Graph qgraph;
      GraphLoader qgraph_loader;
      std::string query_vfile;
      get_query_vfile(query_vfile, index);
      std::string query_efile;
      get_query_efile(query_efile, index);
      std::unordered_map<VertexID, std::unordered_set<VertexID>> orgin_sim;
      dualsim.dual_simulation(dgraph, qgraph, origin_sim, false);
      ...
      for (int extend = 0; extend < extend_num; extend++) {
        ...
        dualsim.dual_simulation(dgraph, qgraph, direct_sim, false);
 ...
 private:
  ...
  std::string rm_name = FLAGS_base_rm_file;
